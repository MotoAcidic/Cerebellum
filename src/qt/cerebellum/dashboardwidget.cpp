...
#include "qt/cerebellum/dashboardwidget.h"
#include "qt/cerebellum/forms/ui_dashboardwidget.h"
#include "qt/cerebellum/sendconfirmdialog.h"
#include "qt/cerebellum/txrow.h"
...
DashboardWidget::DashboardWidget(CEREBELLUMGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of Cerebellum and MN rewards."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareC111, "square-chart-1111");
    setCssProperty(ui->labelSquarezC111, "square-chart-z1111");
    setCssProperty(ui->labelC111, "text-chart-1111");
    setCssProperty(ui->labelZc111, "text-chart-z1111");
    ...
    ui->labelAmountZc111->setText("0 Cerebellum MNR");
    ui->labelAmountC111->setText("0 Cerebellum");
    setCssProperty(ui->labelAmountC111, "text-stake-1111-disable");
    setCssProperty(ui->labelAmountZc111, "text-stake-z1111-disable");
    ...
    ui->btnHowTo->setText(tr("How to get Cerebellum"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("cerebellum")
    updateDisplayUnit();
}
...
// pair cerebellum, cerebellum MNR
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isC111 = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
        bool isMNR = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair cerebellum, cerebellum MNR
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            c111 = (pair.first != 0) ? pair.first / 100000000 : 0;
            zc111 = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalC111 += pair.first;
            chartData->totalZc111 += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesC111.append(c111);
        chartData->valueszC111.append(zc111);

        int max = std::max(c111, zc111);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("Cerebellum");
    set1 = new QBarSet("Cerebellum MNR");
    set0->setColor(QColor("#99CCCC"));
    set1->setColor(QColor("#88c7ff"));
    ...
    set0->append(chartData->valuesC111);
    set1->append(chartData->valueszC111);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalC111 > 0 || chartData->totalZc111 > 0) {
        setCssProperty(ui->labelAmountC111, "text-stake-1111");
        setCssProperty(ui->labelAmountZc111, "text-stake-z1111");
    } else {
        setCssProperty(ui->labelAmountC111, "text-stake-1111-disable");
        setCssProperty(ui->labelAmountZc111, "text-stake-z1111-disable");
    }
    forceUpdateStyle({ui->labelAmountC111, ui->labelAmountZc111});
    ui->labelAmountC111->setText(GUIUtil::formatBalance(chartData->totalC111, nDisplayUnit));
    ui->labelAmountZc111->setText(GUIUtil::formatBalance(chartData->totalZc111, nDisplayUnit) + " MNR");
