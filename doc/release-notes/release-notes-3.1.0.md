CEREBELLUM Core version *3.1.0* is now available from:  <https://github.com/cerebellum-project/cerebellum/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/cerebellum-project/cerebellum/issues>


Mandatory Update
==============

CEREBELLUM Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zC111 Staking is here! zC111 staking will be activated on the morning of the 8th of May 2018. With the release of zC111 staking, there are effectively 2 versions of zC111, zC111 minted on the 3.0.6 CEREBELLUM wallet or lower, and zC111 minted on CEREBELLUM wallet version or higher. New features in this release will require the use of zC111 v2, zC111 minted on this wallet release 3.1.0 or later. If you currently hold zC111 v1 and wish to take advantage of zC111 staking and deterministic zC111, you will need to spend the zC111 v1 to yourself and remint zC111 v2.
Note: To find your zC111 version, click the privacy tab, then the zC111 Control button then expand the arrows next to the desired denomination.


### Deterministic zC111 Seed Keys

zC111 is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zC111 outside of the wallet that the zC111 had been minted on. zC111 can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zC111 minting

zC111 minting now only requires 1 further mint (down from 2) to mature. zC111 mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zC111 Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zC111 denomination has been spent or not.



C111/zC111 Staking and Masternode Rewards
--------------

### C111, zC111 and Masternode Payment Schedule

To encourage the use of zC111 and increase the CEREBELLUM zerocoin anonymity set, the CEREBELLUM payment schedule has been changed to the following:

If a user staking zC111 wins the reward for their block, the following zC111 reward will be: 
- 3 zC111 (3 x 1 denominations) rewarded to the staker, 2 C111 rewarded to the masternode owner and 1 C111 available for the budget. This is a total block reward of 6 C111, up from 5.

If a user staking C111 wins the reward, the following amounts will be rewarded: 
- 2 C111 to the C111 staker, 3 C111 to the Masternode owner and 1 C111 available for the budget. This is a total block reward of 6 C111, up from 5.


### Return change to sender when minting zC111

Previously, zC111 minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most C111 and return the change from a zC111 mint to that address.
...
The visual layout of the CEREBELLUM Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zC111 minting feature. This is enabled by default and the enablezeromint=0 setting in the cerebellum.conf file will overwrite the GUI option.
-	The percentage of autominted zC111 can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular C111.
...
### Summary
 ...
 The CEREBELLUM wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the cerebellum.conf file, found in the CEREBELLUM data directory.
- backuppath = <directory / full path>
- zc111backuppath = <directory / full path>
...
* If zC111 backup, auto generated name is `wallet-autozc111backup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
CEREBELLUM Daemon & Client (RPC Changes)
--------------
...
This allows running cerebellumd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the CEREBELLUM support group, which often times sees users say "I didn't spend that zC111". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the CEREBELLUM masternodes, the monthly budget available to be utilised has been increased to 42,000 C111 / month. This C111 only has the opportunity to be raised once per month (paid to winning proposals) with any unused C111 not created by the blockchain.

### Budget Finalization Fee

The CEREBELLUM finalization fee for successful proposals has now been reduced, this fee is now 5 C111 down from 50 C111. The total fee outlay for a successful proposal is now a total of 55 C111.
...
-	Enforced v1 zC111 spends to require a security level of 100
-	Updates to zC111 spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the CEREBELLUM-Qt masternodes tab
-	Updated CEREBELLUM-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and CEREBELLUM's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zC111 Control dialog in the QT wallet, a user is able to see zC111 marked as version 1 or 2.

zPoS (zC111 staking)
--------------

Once a zC111 has over 200 confirmations it becomes available to stake. Staking zC111 will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zC111. So for example if a 1,000 zC111 denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zC111s.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within CEREBELLUM's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zC111, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zC111 that can be recalculated at any time using the seed. Deterministic zC111 allows for users to backup all of their future zC111 by simply recording their seed and keeping it in a safe place (similar to backing up a private key for C111). The zC111 seed needs to remain in the wallet in order to spend the zC111 after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zC111 data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

CEREBELLUM's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which CEREBELLUM used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to CEREBELLUM using the bug bounty program. The modulus is now correctly imported and CEREBELLUM's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zC111 v2: zPoS dzC111 ezC111 external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zC111 balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zC111 status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zC111 Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault cerebellum-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zC111 spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zC111. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zC111. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/cerebellum-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
