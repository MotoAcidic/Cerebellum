...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("cerebellum-wallet");
...
bool CWalletDB::WriteZC111Seed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZC111Seed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZC111Seed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZC111Seed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZC111Seed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZC111Seed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZC111Count(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZC111Count(uint32_t& nCount)
{
