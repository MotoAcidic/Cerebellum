...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(consensus.IsStakeModifierV2(nHeight))
        pblock->nVersion = 4;       //!> Hardfork version
    else
        pblock->nVersion = 3;
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zC111 spends can have very large priority, use non-overflowing safe functions
        ...
        if (!fProofOfStake) {
            ...
            if (txNew.vout.size() > 1) {
                ...
                CAmount blockValue = GetBlockValue(nHeight,nFees);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CEREBELLUMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("CEREBELLUMMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("CEREBELLUMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("cerebellum-miner");
    const int64_t nSpacingMillis = 60 * 1000;
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            while ((vNodes.empty() && Params().MiningRequiresPeers()) || pwallet->IsLocked() || !fStakeableCoins ||
                ...
                if (!fStakeableCoins) CheckForCoins(pwallet, 0.5);
            }
        ...
        LogPrintf("Running CEREBELLUMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("CEREBELLUMMiner exception");
    } catch (...) {
        LogPrintf("CEREBELLUMMiner exception");
    }

    LogPrintf("CEREBELLUMMiner exiting\n");
}
