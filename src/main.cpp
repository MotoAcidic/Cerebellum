...
#if defined(NDEBUG)
#error "CEREBELLUM cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uc111) are considered zero fee (for relaying and mining)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (!CheckTransaction(tx, false, true, state, false, fColdStakingActive))
...

    {
        ...
        bool fCLTVIsActivated = sporkManager.IsSporkActive(SPORK_19_RHF_ACTIVE);
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
    ...
    if (!CheckTransaction(tx, chainHeight >= false, true, state))
...
{
   return Params().GetConsensus().nMasternodeCollateral;
}
...
{
    if (nHeight <= 100)
        return 1111111111.11 * COIN + nFees;
    CAmount nSubsidy = 10 * COIN; // Default PoS reward
...
{
    util::ThreadRename("cerebellum-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (fScriptChecks && pindex->pprev) {
        fCLTVIsActivated = sporkManager.IsSporkActive(SPORK_19_RHF_ACTIVE);
    }
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->nHeight,nFees);
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        if (!Params().GetConsensus().IsStakeModifierV2(pindexNew->nHeight) && pindexNew->nHeight > Params().GetConsensus().height_last_PoW) {
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after height_start_StakeModifierV2 so GetMasternodePayment is always 3 cerebellum.
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if(pindexPrev->nHeight +1 ==3000)
        return true;//Bypass last pow block check for diff
    ...
    if (block.nBits != nBitsRequired) {
        // Cerebellum Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nCerebellumBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nCerebellumBadBlockBits)) {
            // accept CEREBELLUM block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> c111Inputs;
        std::vector<CTxIn> zC111Inputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                c111Inputs.push_back(stakeIn);
        }
        const bool hasC111Inputs = !c111Inputs.empty();
        const bool hasZC111Inputs = !zC111Inputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasC111Inputs) {
                    ...
                    for (const CTxIn& c111In : c111Inputs)
                        if(c111In.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasC111Inputs) {
                            for (const CTxIn& stakeIn : c111Inputs)
