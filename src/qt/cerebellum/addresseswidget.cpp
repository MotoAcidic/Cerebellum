...
#include "qt/cerebellum/forms/ui_addresseswidget.h"
#include "qt/cerebellum/addresslabelrow.h"
#include "qt/cerebellum/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(CEREBELLUMGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter Cerebellum address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress c111Add = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(c111Add.Get(), label.toUtf8().constData(),
                c111Add.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
