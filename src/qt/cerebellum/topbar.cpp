...
#include "qt/cerebellum/topbar.h"
#include "qt/cerebellum/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/cerebellum/lockunlock.h"
#include "qt/cerebellum/qtutils.h"
#include "qt/cerebellum/receivedialog.h"
...
TopBar::TopBar(CEREBELLUMGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezC111, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzC111, ui->labelTitleImmaturezC111};
    ...
    setCssProperty({ui->labelAmountTopC111, ui->labelAmountTopzC111}, "amount-small-topbar");
    setCssProperty({ui->labelAmountC111, ui->labelAvailablezC111}, "amount-topbar");
    setCssProperty({ui->labelPendingC111, ui->labelPendingzC111, ui->labelImmatureC111, ui->labelImmaturezC111}, "amount-small-topbar");
    ...
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        if (displayUnitPrev != nDisplayUnit)
            updateBalances(walletModel->getBalance(), walletModel->getUnconfirmedBalance(), walletModel->getImmatureBalance(),
                           0/*zerocoinbalance*/, 0/*unconfzerocoinbalance*/, 0/*immaturezerocinbalance*/,
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // cerebellum Total
    CAmount c111AvailableBalance = balance;
    // zC111 Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalC111 = GUIUtil::formatBalance(c111AvailableBalance, nDisplayUnit);
    QString totalzC111 = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // cerebellum
    // Top
    ui->labelAmountTopC111->setText(totalC111);
    // Expanded
    ui->labelAmountC111->setText(totalC111);
    ui->labelPendingC111->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureC111->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zC111 balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zC111 label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzC111->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzC111->setText(totalzC111);
    // Expanded
    ui->labelAvailablezC111->setText(totalzC111);
    ui->labelPendingzC111->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezC111->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
