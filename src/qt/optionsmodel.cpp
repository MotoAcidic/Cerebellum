...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::C111);
    ...
    if (settings.contains("nAnonymizeCerebellumAmount") || reset)
        SoftSetArg("-anonymizecerebellumamount", settings.value("nAnonymizeCerebellumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cerebellum.cpp during shotdown to also remove the window positions
