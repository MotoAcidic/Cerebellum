...
enum WalletFeature {
    ...
    FEATURE_PRE_CEREBELLUM = 61000, // inherited version..
    ...
    FEATURE_PRE_SPLIT_KEYPOOL = 2000000, // Upgraded to HD SPLIT and can have a pre-split keypool
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 cerebellum at the same time
    ONLY_4000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for zC111 send
enum ZerocoinSpendStatus {
    ZC111_SPEND_OKAY = 0,                            // No error
    ZC111_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZC111_WALLET_LOCKED = 2,                         // Wallet was locked
    ZC111_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZC111_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZC111_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZC111_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZC111_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZC111_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZC111_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZC111_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZC111_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZC111_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZC111_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZC111_SPENT_USED_ZC111 = 14,                      // Coin has already been spend
    ZC111_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZC111_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 cerebellum output and keys which can be used for the Masternode
