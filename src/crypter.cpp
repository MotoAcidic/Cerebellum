    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZC111Seed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZC111Seed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zc111seed to wallet";
    }
                //the use case for this is no password set seed, mint dzC111,
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZC111Seed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZC111Seed(hashSeed, vchSeed)) {
