...
/**
 * Fees smaller than this (in uc111) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(&vPossibleCoins, true, NULL, false, ONLY_4000, false, 1, false, false);
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (  (mine == ISMINE_NO) ||
                      ...
                      (IsLockedCoin((*it).first, i) && nCoinType != ONLY_4000) ||
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 cerebellum.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 cerebellum.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-cerebellum-address
        ...
        )
{
    ...
    // Parse utxos into CC111Stakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CC111Stake> input(new CC111Stake());
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        // Calculate reward
        CAmount nReward;
        nReward = GetBlockValue(chainActive.Height() + 1,0);
        nCredit += nReward;
