...
#include "qt/cerebellum/send.h"
#include "qt/cerebellum/forms/ui_send.h"
#include "qt/cerebellum/addnewcontactdialog.h"
#include "qt/cerebellum/qtutils.h"
#include "qt/cerebellum/sendchangeaddressdialog.h"
#include "qt/cerebellum/optionbutton.h"
#include "qt/cerebellum/sendconfirmdialog.h"
#include "qt/cerebellum/myaddressrow.h"
#include "qt/cerebellum/guitransactionsutils.h"
...
SendWidget::SendWidget(CEREBELLUMGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (cerebellum)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("cerebellum address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 cerebellum");
...
void SendWidget::refreshAmounts() {
    ...
    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZc111));
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendC111 ? AskPassphraseDialog::Context::Send_C111 : AskPassphraseDialog::Context::Send_zC111, true)){
...
void SendWidget::onCoinControlClicked(){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any cerebellum to select."));
        }
}
...
void SendWidget::onC111Selected(bool _isC111){
    isC111 = _isC111;
    setCssProperty(coinIcon, _isC111 ? "coin-icon-c111" : "coin-icon-zc111");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress c111Add = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(c111Add)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(c111Add.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
