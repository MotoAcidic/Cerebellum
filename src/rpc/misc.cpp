...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total cerebellum balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in cerebellum/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in cerebellum/kb\n"
...
UniValue createsporkkeypair(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"cerebellumaddr\" : \"cerebellumaddress\", (string) The cerebellum public address \n"
    ...
    if (isValid) {
        ...
        //get public address
        ret.push_back(Pair("cerebellumaddr", currentAddress));
        CScript scriptPubKey = GetScriptForDestination(dest);
        //get scriptpubkey
        CPubKey vchPubKey;
        pwalletMain->GetPubKey(keyID, vchPubKey);
        ret.push_back(Pair("strsporkpubkey", HexStr(vchPubKey)));
        //get privkey
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"cerebellumaddress\"\n"
            "\nReturn information about the given cerebellum address.\n"

            "\nArguments:\n"
            "1. \"cerebellumaddress\"     (string, required) The cerebellum address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cerebellumaddress\",    (string) The cerebellum address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for CEREBELLUM cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: cerebellum address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are cerebellum addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) cerebellum address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"cerebellumaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"cerebellumaddress\"  (string, required) The cerebellum address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,    (boolean) if staking is enabled/disabled in cerebellum.conf\n"
