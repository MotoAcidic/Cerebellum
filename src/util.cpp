...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cerebellum" is a composite category enabling all CEREBELLUM-related debug output
            if (ptrCategory->count(std::string("cerebellum"))) {
...
#else
    const char* pszModule = "cerebellum";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CEREBELLUM
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CEREBELLUM
// Mac: ~/Library/Application Support/CEREBELLUM
// Unix: ~/.cerebellum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cerebellumDataNew";
#else
    fs::path pathRet;
    ...
    return pathRet / "cerebellumDataNew";
#else
    // Unix
    return pathRet / ".cerebellumDataNew";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cerebellum.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cerebellum.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cerebellum.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cerebellumd.pid"));
