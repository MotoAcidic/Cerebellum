...
bool CC111Stake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CC111Stake::GetTxFrom(CTransaction& tx) const
{
    tx = txFrom;
    return true;
}

bool CC111Stake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

int64_t CC111Stake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CC111Stake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CC111Stake::GetUniqueness() const
{
    //The unique identifier for a C111 stake is the outpoint
...
CBlockIndex* CC111Stake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
